...
# Merge the two subject arrays
subjectsTotal=("${subjects[@]}")

data_dir="/Volumes/ExtremeSSD/LIBR_tACS/Pre"
stim_folder="/Volumes/ExtremeSSD/LIBR_tACS/Preprocessing_pre/stim_folder_tACS"
session_id="MCR1"

# Process fMRI data for each subject
...
do
    ...
    afni_proc.py -subj_id $task_id \
	    -dsets $data_dir/$SUBJECT/func/sub-${SUBJECT}_ses-v1_task-opioidcuereactivity1_bold.nii.gz \
            ...
            -regress_basis_multi 'BLOCK(31,1)' 'BLOCK(31,1)' \
            -regress_stim_times $stim_folder/opioid-R1.1D  $stim_folder/neutral-R1.1D  \
            -execute \
            -remove_preproc_files \
            -regress_stim_labels O N \
            -regress_opts_3dD -bout -nfirst 3 -gltsym 'SYM: O -N' -glt_label 1 "OvN"
done